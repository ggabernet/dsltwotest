# nf-core/dsltwotest: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/dsltwotest/usage](https://nf-co.re/dsltwotest/usage)
...
```bash
nextflow run nf-core/dsltwotest --input samplesheet.csv -profile docker
```
...
```bash
nextflow pull nf-core/dsltwotest
```
...
First, go to the [nf-core/dsltwotest releases page](https://github.com/nf-core/dsltwotest/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
...
* `docker`
  ...
  * Pulls software from Docker Hub: [`nfcore/dsltwotest`](https://hub.docker.com/r/nfcore/dsltwotest/)
* `singularity`
